...
async function handler(req, res) {
  ...
  function isValidPlay(cards, top) {
    ...
    if (game.fortActive && game.turn !== game.fortOwner) {
      ...
      return false; // Only pairs can challenge fort for non-owners
    }
    ...
    if (!top && game.phase === 'play') {
      if ((rulerRank === 'A' && rulerSuit === 'Diamonds') && cards.every(c => !['J', 'Q', 'K'].includes(c.rank) && rankValue(c.rank) % 2 !== 0)) return !isPair;
      if ((rulerRank === '3' || rulerRank === '7') && cards.length === 1 && (cards[0].rank === '7' || cards[0].rank === '3')) return true;
      if (rulerRank === '10' && cards.length >= 2 && cards.every(c => isEven(c.rank))) return !isPair;
    ...
    if (cards.length === 1) {
      ...
      if ((rulerRank === 'A' && rulerSuit === 'Spades')) matches = matches || Math.floor(value / 2) === topValue;
      ...
      if (rulerSuit === 'Hearts' && rulerRank !== 'A') matches = matches || rankValue(rulerRank) === topValue || rankValue(rulerRank) % 2 === topValue % 2;
      if (rulerSuit === 'Spades' && rulerRank !== 'A' && card.suit === 'Spades') matches = matches || Math.ceil(rankValue(card.rank) / 2) - 1 === topValue || Math.ceil(rankValue(card.rank) / 2) - 1 % 2 === topValue % 2;
      if (rulerSuit === 'Clubs' && game.players[game.turn].hand.length >= 5) matches = true; // Strike ability
      return matches;
    }
    ...
    if (rulerRank === '10' && cards.length >= 2 && cards.every(c => isEven(c.rank)) && isEven(top.rank)) return !isPair;
